...
async def build_profile_image(inter: disnake.MessageInteraction, group_choice: str, пользователь: disnake.Member) -> str:
    """Собирает изображение профиля и возвращает путь до файла."""
    now = datetime.now()
    ...
    avatar_bytes = await пользователь.display_avatar.read()
    avatar_image = Image.open(BytesIO(avatar_bytes)).convert("RGBA").resize((width, height))

    mask_im = Image.new("L", (width, height))
    ImageDraw.Draw(mask_im).ellipse((0, 0, width, height), fill=255)
    image.paste(avatar_image, (avatar_x, avatar_y), mask_im)
    ...
    async def on_button_click(self, inter):
        ...
        if custom_id.endswith("profile_action"):
            ...
            if custom_id == "profile_action":
                ...
                path = await build_profile_image(inter, group_choice, пользователь)
                return await inter.message.edit(
                    attachments=None,
                    file=disnake.File(path),
                    view=ProfileStaffView(inter, None, inter.author.id, group_choice)
                )

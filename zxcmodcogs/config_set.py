...
class BotConfig(commands.Cog):
    ...
    async def on_button_click(self, inter: disnake.MessageInteraction):
        ...
        if cid in {"roles_create", "channels_create", "categories_create", "logs_create"}:
        ...
        if cid == "roles_create":
            ...
            await inter.followup.send("Недостающие роли созданы.", ephemeral=True)
        ...
        if cid == "channels_create":
            ...
            await inter.followup.send("Недостающие каналы созданы.", ephemeral=True)

        if cid == "categories_create":
            for key in miss["categories"]:
                cat = await ctx.guild.create_category(key)
                self.config[key] = cat.id
            await inter.followup.send("Недостающие категории созданы.", ephemeral=True)
        ...
        if cid == "logs_create":
            ...
            await inter.followup.send("Недостающие лог-каналы созданы.", ephemeral=True)

        ...
        if custom_id[-6:] == 'report':
            if custom_id == 'accept_report':
                ...
                embed = disnake.Embed(
                    ...
                    description=(
                        f"* {one} - переместить {user.mention}\n"
                        f"* {two} переместить {target.mention}\n"
                        f"* {three} - завершить в пользу {user.mention}\n"
                        f"* {four} - завершить в пользу {target.mention}\n"
                        f"* {basket} Закрыть тикет"
                    ),
                    color = 3092790
                )

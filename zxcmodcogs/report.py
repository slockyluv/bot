...
class ReportMenu(disnake.ui.View):
    def __init__(self):
        ...
        emojis = {
            ...
            "three": files.find_one({"_id": "three"})["emoji_take"],
            "four": files.find_one({"_id": "four"})["emoji_take"],
            "action_basket": files.find_one({"_id": "action_basket"})["emoji_take"],
        }

        self.add_item(disnake.ui.Button(style = ButtonStyle.secondary, emoji = emojis["one"], custom_id = 'move_one_report', row = 0))
        self.add_item(disnake.ui.Button(style = ButtonStyle.secondary, emoji = emojis["two"], custom_id = 'move_two_report', row = 0))
        self.add_item(disnake.ui.Button(style = ButtonStyle.secondary, emoji = emojis["three"], custom_id = 'accept_one', row = 0))
        self.add_item(disnake.ui.Button(style = ButtonStyle.secondary, emoji = emojis["four"], custom_id = 'accept_two', row = 0))
        self.add_item(disnake.ui.Button(style = ButtonStyle.red, emoji = emojis["action_basket"], custom_id = 'close_report', row = 0))
    ...
    async def on_button_click(self, inter):
        ...
        if custom_id[-6:] == 'report':
            if custom_id == 'accept_report':
                ...
                description = (
                    f"* {emojis['one']} - переместить {user.mention}\n"
                    f"* {emojis['two']} переместить {target.mention}\n"
                    f"* {emojis['three']} - завершить в пользу {user.mention}\n"
                    f"* {emojis['four']} - завершить в пользу {target.mention}\n"
                    f"* {emojis['action_basket']} Закрыть тикет"
                )

                embed = disnake.Embed(title = f"Управление жалобой {number}", description=description, color = 3092790)
